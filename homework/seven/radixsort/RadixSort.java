...
public class RadixSort {
	...
	public static void sort(IPAddress[] elements) {
		...
		for (int exp = 1; max.getIpFrom() / exp > 0; exp *= 10) {		// 2
			sort(elements, exp);										// 3
		}
	}
	...
	private static void sort(IPAddress[] elements, int exp) {
		...
		for (int i = 0; i < elements.length; i++) {						// 3
			int digit = (int) ((elements[i].getIpFrom() / exp) % 10);	// 3
			frequency[digit]++;											// 3
		}
		...
		for (int i = elements.length - 1; i >= 0; i--) {				// 5
			int digit = (int) ((elements[i].getIpFrom() / exp) % 10);	// 5
	...
	private static IPAddress getMax(IPAddress[] elements) {
		...
		for (int i = 1; i < elements.length; i++) {						// 2
			if (elements[i].getIpFrom() > max.getIpFrom()) {			// 3
				max = elements[i];										// 3
			}
		}
		return max;														// 4
